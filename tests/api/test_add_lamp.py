...
async def test_add_lamp_201(
    xclient: AsyncClient, test_db: DatabaseConnector, manufacturer: Manufacturer
):
    payload = {
        "article": 440101,
        "price": 832.6,
...
async def test_add_lamp_404(
    ...
    assert response.json() == {"detail": "The lamp with the article already exists."}


@pytest.mark.asyncio
@pytest.mark.usefixtures("prepare_manufacturer")
@pytest.mark.usefixtures("prepare_lamp")
async def test_add_lamp_404_1(
    xclient: AsyncClient,
    test_db: DatabaseConnector,
    lamp: Lamp,
    manufacturer: Manufacturer,
    ...
    response = await xclient.post("/lamp/", json=payload)
    assert response.status_code == 404, response.text
    assert response.json() == {"detail": "There is no manufacturer with such an ID."}


@pytest.mark.asyncio
@pytest.mark.usefixtures("prepare_manufacturer")
async def test_add_lamp_non_type_422_price(
    xclient: AsyncClient, test_db: DatabaseConnector, manufacturer: Manufacturer
):
    payload = {
        "article": 440101,
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("prepare_manufacturer")
async def test_add_lamp_non_type_422_article(
    xclient: AsyncClient, test_db: DatabaseConnector, manufacturer: Manufacturer
):
    payload = {
        "article": "asd",
